...
def main():
    # Read the CSV file
    df = pd.read_csv('/Users/gaurav/setbull_projects/setbull_trader/python_strategies/backtest_results/daily_trades.csv')
    
    # Convert Date column to datetime
    df['Date'] = pd.to_datetime(df['Date'])
    
    # Define time periods
    entries = [5, 10, 15, 30, 50, 100]
    
    # Generate timestamp for unique filename
    timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    filename = f"stock_performance_report_{timestamp}.txt"
    filepath = f"/Users/gaurav/setbull_projects/setbull_trader/python_strategies/results/mr_strategy_agg/{filename}"
